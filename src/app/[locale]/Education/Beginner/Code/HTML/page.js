import Styles from "./HTML-Education-Beginner.module.css";
import ExampleBox from "./ClientComponent/example-box/example-box";
import Navbar from "@/app/components/Navbar/navbar";
import Footer from "@/app/components/Footer/footer";
import MWH from "@/app/components/MWH/mwh";
import SideBar from "@/app/components/SideBar/SideBar";

export default async function HTMLEducationBeginner({ params }) {
  ...
  const {
    ...
    example,
    paragraft,
  } = require(`./locale/${locale}/locale.json`);

  return (
    <>
      <Navbar />
      <div className={Styles.BigMainContainer}>
        <SideBar />
        <div className={Styles.HTML_Education_Beginner}>
          <main className={Styles.main}>
            <section className={Styles.hero}>
              {" "}
              <h1 className={Styles.title}>HTML</h1>
              <p className={Styles.description}>Hyper Text Markup Language</p>
              <svg
                className={Styles.icon}
                version="1.1"
                xmlns="http://www.w3.org/2000/svg"
                xmlnsXlink="http://www.w3.org/1999/xlink"
                viewBox="0 0 191.613 191.614"
                xmlSpace="preserve"
              >
                <g>
                  <path
                    d="M65.804,5.585c-0.57-2.445-4.569-2.569-5.053,0c-0.365,1.943-0.492,3.922-0.612,5.899c-2.033-0.625-4.737-0.043-6.675,0.06
		...
		c-0.393,1.915-1.002,3.72-1.331,4.864c-0.327,1.139,0.185,2.565,1.27,3.12C67.613,29.149,66.651,9.225,65.804,5.585z"
                  />
                  <path
                    d="M92.981,4.747c-2.301-2.67-7.693-1.751-10.851-1.64c-4.511,0.159-8.961,0.409-13.422,1.014c-1.774,0.24-1.333,2.67,0,3.16
		...
		c-0.019-4.853-0.263-9.668-0.75-14.483c3.15,0.36,7.033,0.886,9.242-0.562C93.907,7.802,94.085,6.029,92.981,4.747z"
                  />
                  <path
                    d="M124.476,19.215c-0.243-4.218-0.571-8.44-0.609-12.668c-0.024-2.81-3.197-3.772-5.163-2.138
		...
		c0.14,2.381-0.386,7.484,3.263,7.484C125.48,26.701,124.611,21.572,124.476,19.215z"
                  />
                  <path
                    d="M146.327,21.488c-2.958,0.518-5.922,0.668-8.902,0.711c0.136-6.575,0.407-13.396-2.16-19.511
		...
		c4.548-0.01,9.016-0.173,13.501-0.968C151.666,26.539,150.063,20.833,146.327,21.488z"
                  />
                  <path
                    d="M159.828,39.732c-18.057,0.021-36.012-0.342-54.084-0.876c-24.188-0.715-48.065,0.567-72.197-2.098
		...
		c-3.944-2.276-9.327-3.224-13.411-4.867C75.888,155.019,69.319,151.53,62.486,149.653z"
            ...
            <section className={Styles.description_1}>
              <p>{paragraft[0]}</p>
              <p className={Styles.question}>{paragraft[1]}</p>
              <p>{paragraft[2]}</p>
              <p className={Styles.question}>{paragraft[3]}</p>
              <p>{paragraft[4]}</p>
              <p>{paragraft[5]}</p>
              <div className={Styles.container_description}>
                <h2>{definition.title}</h2>
                <p>
                  <span className={Styles.highlight_in_description}>
              ...
              <ExampleBox exampleContent={example} />
              <p>{paragraft[7]}</p>
              <p>{paragraft[8]}</p>

              <p>{paragraft[9]}</p>
              <div className={Styles.container_button_link_get_started}>
                <a className={Styles.linkGetStarted} href="#">
                  Mulai belajar
                </a>
              </div>
            </section>
